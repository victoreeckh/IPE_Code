...
def get_electricity_bill(total_consumption_profile, total_consumption, consumption_day, consumption_night, total_injection_profile, injection_day, injection_night, dynamic_tarrif, EV_case):
...
total_consumption_profile = P_offtake.flatten().tolist()
total_injection_profile = P_injection.flatten().tolist()
total_consumption = np.sum(P_offtake.flatten())

P_offtake = P_offtake.reshape(365,24,4)
all_indices = np.arange(365)
mask = np.logical_or((all_indices % 6 == 0),(all_indices % 6 == 5))
P_offtake_weekends = P_offtake[mask,:,:]
P_offtake_weekdays = P_offtake[1-mask,:,:]

consumption_day = np.sum(P_offtake_weekdays[:,6:21,:].flatten(),axis=0)
consumption_night = np.sum(P_offtake_weekends.flatten(),axis=0) + np.sum(P_offtake_weekdays[:,:6,:].flatten(),axis=0) + np.sum(P_offtake_weekdays[:,21:,:].flatten(),axis=0)

P_injection = P_injection.reshape(365,24,4)
P_injection_weekends = P_injection[mask,:,:]
P_injection_weekdays = P_injection[1-mask,:,:]

injection_day = np.sum(P_offtake_weekdays[:,6:21,:].flatten(),axis=0)
injection_night = np.sum(P_injection_weekends.flatten(),axis=0) + np.sum(P_injection_weekdays[:,:6,:].flatten(),axis=0) + np.sum(P_injection_weekdays[:,21:,:].flatten(),axis=0)

print(f'{get_electricity_bill(total_consumption_profile, total_consumption, consumption_day, consumption_night, total_injection_profile, injection_day, injection_night, False, False):.2f}')
print(f'{get_electricity_bill(total_consumption_profile, total_consumption, consumption_day, consumption_night, total_injection_profile, injection_day, injection_night, True, False):.2f}')
print(f'{get_electricity_bill(total_consumption_profile, total_consumption, consumption_day, consumption_night, total_injection_profile, injection_day, injection_night, False, True):.2f}')
print(f'{get_electricity_bill(total_consumption_profile, total_consumption, consumption_day, consumption_night, total_injection_profile, injection_day, injection_night, True, True):.2f}')
